...
summary_file="radical-probablism.txt"
main_dir=$(pwd)
...
process_files() {
    ...
    for file in "$dir"/*.txt; do
        ...
        if [ -f "$file" ]; then
            ...
            local file_base="${file_name%.*}"    # Get the file name minus extension
            ...
            if ! is_processed "$file_name"; then
                ...
                # Add the document name as a heading (once per document)
                echo "" >> "$main_dir/$summary_file"
                echo "===== $file_base =====" >> "$main_dir/$summary_file"
                echo "" >> "$main_dir/$summary_file"
